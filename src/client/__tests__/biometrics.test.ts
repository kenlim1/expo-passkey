...
describe('Biometrics Utilities', () => {
  ...
  beforeEach(() => {
    ...
    mockLocalAuthentication.supportedAuthenticationTypesAsync.mockResolvedValue([2]); // Face ID by default
  ...
  describe('checkBiometricSupport', () => {
    ...
    test('returns correct info for supported iOS device with Touch ID', async () => {
      // Change to Touch ID
      mockLocalAuthentication.supportedAuthenticationTypesAsync.mockResolvedValue([1]); // Touch ID
    ...
    test('returns correct info for Android device with Face Unlock', async () => {
      ...
      mockLocalAuthentication.supportedAuthenticationTypesAsync.mockResolvedValue([2]); // Facial recognition
    ...
    test('returns correct info for Android device with Iris scanning', async () => {
      ...
      mockLocalAuthentication.supportedAuthenticationTypesAsync.mockResolvedValue([3]); // Iris
    ...
    test('handles errors gracefully', async () => {
      ...
      mockLocalAuthentication.hasHardwareAsync.mockRejectedValue(new Error('Test error'));
  ...
  describe('authenticateWithBiometrics', () => {
    test('returns true when authentication succeeds on iOS', async () => {
      ...
      expect(mockLocalAuthentication.authenticateAsync).toHaveBeenCalledWith(options);
    });
    ...
    test('throws error when authentication fails', async () => {
      ...
      try {
        ...
        expect((error as { code: string }).code).toBe(ERROR_CODES.BIOMETRIC.AUTHENTICATION_FAILED);
      }
    });
